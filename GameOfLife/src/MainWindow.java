...
public class MainWindow extends JFrame {
	...
	private static final Border DEFAULT_BORDER = BorderFactory.createLineBorder(Color.BLUE);
	
	private static final Font DEFAULT_GUI_FONT = new Font("Unifont", Font.PLAIN, 12);
	private static final Font INFO_FONT = new Font("Unifont", Font.BOLD, 14);
	private static final Font STATUS_FONT = new Font("Unifont", Font.BOLD, 12);
	private static final Font SLIDER_FONT = new Font("Unifont", Font.BOLD, 12);
	...
	private JPanel[][] grid;
	private JButton init, start, stop, plusOne, reset,savedb;
	private int[][] cells;
	...
	public void initGrid() {
		gridPanel.setBackground(DEFAULT_GUI_BACKGROUND);//it isn't visible due to jpanel overriding
			...
			for(int j=0; j<grid[i].length; j++) {
				...
				grid[i][j].addMouseListener(new MouseListener(){
					...
					public void mouseClicked(MouseEvent e) {
						...
						if(!inProgress) {
							flipCell(x ,y);
							if(!start.isEnabled()) { //isEnable makes button click-able or not
	...
	public void initSettings() {
		...
		savedb = new JButton("Save State");
		//currently saving in the database is not supported yet but will be made available soon
		...
		savedb.setFont(DEFAULT_GUI_FONT);
		
		start.setEnabled(false);
		stop.setEnabled(false);
		plusOne.setEnabled(false);
		savedb.setEnabled(false);//check if game is in progeess then only enables the saving of database
		...
		topPanel.setLayout(new GridLayout(1, 3, 0, 0));//parms are rows cols horigap vertigap
		topPanel.add(init);
		topPanel.add(reset);
		topPanel.add(savedb);
		
		JPanel temp = new JPanel();
		temp.setLayout(new GridLayout(2, 1, 0, 0));//parms are rows cols horigap vertigap
	...
	private JPanel getStatsPanel() {
		...
		temp.add(getStylizedLabel("Tip:Cells can be Curosr-selected or randomly initialized"));
		temp.add(getStylizedLabel("----- ----- ----- ----- ----- ----- ----- ----- ----- -----"));
		temp.add(cgLabel);
		temp.add(cpLabel);
		temp.add(mxpLabel);
	...
	private JPanel getControlPanel() {
		...
		wrapGridCheck.addChangeListener(new ChangeListener() {//wrapping isnt supported yet
			public void stateChanged(ChangeEvent e) {
				if(!inProgress) {
	...
	//file handling for saving state information
	private void saveToFile()
	{
		try {
	        File myObj = new File("LastSaved.txt");
		...
		///done  with creaton of file now les write data into it
		try {
		      FileWriter myWriter = new FileWriter("LastSaved.txt");
		      //wriritng meta data to file
		      int[][] nextGeneration = new int[cells.length][cells.length];
				for(int i=0; i<nextGeneration.length; i++) {
					for(int j=0; j<nextGeneration[i].length; j++) {
							//main loops
						if(isAlive(i, j))
						{
							myWriter.write(49);//here you gaive the ascii of what to be wittern
							myWriter.write(32);
							myWriter.write(32);
						}
						else {
